...
// 'I told Fred I wanted to steal his lovely CLI and he said "please do"' -Chance
...
async function getContext(argv: string[]): Promise<Context> {
  ...
  if (selectedRemixVersion) {
    ...
    // TODO: Probably want to check to see if this version of Remix actually exists!
  }
...
async function installDependenciesQuestionStep(ctx: Context) {
  ...
  ctx.pkgManager = (process.env.npm_config_user_agent ?? "npm").split("/")[0];
...
async function runInitScriptQuestionStep(ctx: Context) {
  ...
  if (ctx.initScript === undefined) {
    let { init } = await ctx.prompt({
      ...
      type: "confirm",
      label: title("init"),
      message: `This template includes a custom init script. Do you want to run it during the install process?`,
      hint: "recommended",
      initial: true,
    });
...
async function installDependenciesStep(ctx: Context) {
  if (!ctx.install) {
    ...
    info(
      ...
      `Remember to install dependencies after setup with ${color.reset(
  ...
  await loadingIndicator({
    ...
    while: () => installDependencies(ctx.pkgManager, ctx.cwd),
    ctx,
  });
}
...
async function runInitScriptStep(ctx: Context) {
  ...
  let initCommand = `${
    pkgManagerExecScript[ctx.pkgManager as keyof typeof pkgManagerExecScript] ||
    pkgManagerExecScript["npm"]
  } remix init`;
  ...
  info("Init script:", "Running template's custom remix.init script...\n");
...
async function gitInitQuestionStep(ctx: Context) {
  ...
  let resovedGit = ctx.git;
  if (ctx.git === undefined) {
    ({ git: resovedGit } = await ctx.prompt({
      name: "git",
      type: "confirm",
      label: title("git"),
...
function isEmpty(dirPath: string) {
  ...
  let VALID_PROJECT_DIRECTORY_SAFE_LIST = [
    ".DS_Store",
    ".git",
    ".gitkeep",
    ".gitattributes",
...
async function updatePackageJSON(ctx: Context) {
  ...
  for (let pkgKey of ["dependencies", "devDependencies"] as const) {
    ...
    if (!isValidJsonObject(dependencies)) {
      ...
      throw new Error(`package.json ${pkgKey} is invalid`);
    }
...
${color.arg("--[no-]init-script")}  ${color.dim(`Whether or not to run the template's custom init script, if present`)}
