...
describe("create-remix CLI", () => {
  ...
  it("supports the --help flag", async () => {
    let { stdout } = await execCreateRemix(["--help"]);
    expect(stdout.trim()).toMatchInlineSnapshot(`
      ...
      --[no-]init-script  Whether or not to run the template's custom init script, if present
  ...
  it("supports the --version flag", async () => {
    let { stdout } = await execCreateRemix(["--version"]);
  ...
  it("allows you to go through the prompts", async () => {
    ...
    let { status } = await interactWithShell(proc, [
      { question: /where.*create.*project/i, type: [projectDir, ENTER] },
      { question: /init.*git/i, type: ["n"] },
      { question: /install dependencies/i, type: ["n"] },
    ]);
  ...
  it("fails for private GitHub username/repo combo without a token", async () => {
    ...
    let { status, stderr } = await interactWithShell(proc, []);
  ...
  it("prompts to run remix.init script when installing dependencies", async () => {
    ...
    let { status, stdout } = await interactWithShell(proc, [
      {
    ...
    expect(status).toBe(0);
    expect(stdout).toContain(`Init script complete`);
    expect(fse.existsSync(path.join(projectDir, "package.json"))).toBeTruthy();
    expect(fse.existsSync(path.join(projectDir, "app/root.tsx"))).toBeTruthy();
  ...
  it("doesn't prompt to run remix.init script when not installing dependencies", async () => {
    ...
    expect(status).toBe(0);
    expect(stdout).not.toContain(`Init script complete`);
  ...
  describe("errors", () => {
    ...
    it("doesn't allow creating an app in a dir if it's not empty and then prompts for an empty dir", async () => {
      ...
      let { status, stdout } = await interactWithShell(proc, [
        { question: /where.*create.*project/i, type: [emptyDir, ENTER] },
      ]);

      expect(status).toBe(0);
      expect(stdout).toContain(`Hmm... "${notEmptyDir}" is not empty!`);
